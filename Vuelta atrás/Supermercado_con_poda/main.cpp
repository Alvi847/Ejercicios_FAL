 ...
 * 
 * La k hace referencia al producto que estoy mirando y la i al supermercado donde estoy mirando ese producto
 */
// función que resuelve el problema
void resolver(int k, const t_datos &d, t_sol &s, vector<int> marcas, int &costeMin, const vector<int>& acumulados)
{
    for (int i = 0; i < d.supermercados; i++)
    {
        ...
        if (marcas[i] < 3)
        {
            ...
            {
                if(s.sumaPrecios + acumulados[k + 1] <= costeMin)
                    resolver(k + 1, d, s, marcas, costeMin, acumulados);
            }
            marcas[i]--;
            s.sumaPrecios -= d.precios[i][k];
        }
...
void resuelveCaso()
{
    ...
    if (datos.supermercados > 0 && datos.productos > 0)
    {
        ...
        vector<int> acumulados(datos.productos, 0);

        // Construyo el vector de acumulados por productos
        for(int i = 0; i < datos.productos; i++){
            int auxMin = datos.precios[0][i];
            for(int j = 0; j < datos.supermercados; j++){
                if(datos.precios[j][i] < auxMin)
                    auxMin = datos.precios[j][i];
            }
            acumulados[i] = auxMin;
        }

        // Termino de construir el vector de acumulados guardando en cada posición la suma de las siguientes posiciones
        for(int i = datos.productos - 1; i > 0; --i){
        ...
        resolver(0, datos, s, marcas, costeMin, acumulados);
        cout << costeMin << endl;
    }
    else
