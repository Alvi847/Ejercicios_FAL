...
int estimarSuperficie(const datos_t &d, const sol_t &s, int k)
{
    int i = k + 1, sumaCoste = s.coste, sumaSuperficie = 0;
    while (i < d.objetos.size() &&
           sumaCoste + d.objetos[i].coste <= d.presupuesto)
    {
        sumaCoste += d.objetos[i].coste;
        sumaSuperficie += d.objetos[i].superficie;
        ++i;
    }
    if (i < d.objetos.size() && sumaCoste < d.presupuesto)
        sumaSuperficie += d.objetos[i].superficie *
                          (d.presupuesto - sumaCoste) / (float)d.objetos[i].coste;
    return sumaSuperficie + s.supActual;
}
...
void resolver(const datos_t &d, sol_t &s, int k)
{
    ...
    if (k == d.numObjetos - 1)
    {
        ...
        if(estimarSuperficie(d, s, k) > s.maxSup)
        resolver(d, s, k + 1);
}
